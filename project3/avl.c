...
/* avl_remove api
use lookup to find that node
go and decrement the count in it

fake remove
change lookup to not take the node that has count as 0

then think of actually removing
now when count becomes 0 we need to free the memory 
2 parts :
1-> removing node and readjusting the nodes
2-> rebalancing the tree, ok if we dont do part 2*/
